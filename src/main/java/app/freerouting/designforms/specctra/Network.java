...
public class Network extends ScopeKeyword
{
  ...
  private static void insert_class_pair_info(NetClass.ClassClass p_class_class, app.freerouting.rules.NetClass p_first_class, app.freerouting.rules.NetClass p_second_class, BasicBoard p_board, CoordinateTransform p_coordinate_transform)
  {
  ...
  private static void add_mixed_clearance_rule(ClearanceMatrix p_clearance_matrix, app.freerouting.rules.NetClass p_first_class, app.freerouting.rules.NetClass p_second_class, Rule.ClearanceRule p_clearance_rule, int p_layer_no, CoordinateTransform p_coordinate_transform)
  {
  ...
  private static void add_clearance_rule(ClearanceMatrix p_clearance_matrix, app.freerouting.rules.NetClass p_net_class, Rule.ClearanceRule p_rule, int p_layer_no, CoordinateTransform p_coordinate_transform)
  {
  ...
  private static void insert_component(ComponentPlacement.ComponentLocation p_location, String p_lib_key, ReadScopeParameter p_par)
  {
    ...
    app.freerouting.board.Component new_component = routing_board.components.add(p_location.name, component_location, rotation_in_degree, p_location.is_front, curr_front_package, curr_back_package, p_location.position_fixed);
    ...
    for (int k = 0; k <= 2; ++k)
    {
      ...
      for (int i = 0; i < keepout_arr.length; ++i)
      {
        ...
        if (layer >= 0)
        {
          ...
          {
            routing_board.insert_via_obstacle(curr_keepout.area, layer, component_translation, rotation_in_degree, !p_location.is_front, clearance_class, new_component.no, curr_keepout.name, fixed_state);
          }
          else
          {
            routing_board.insert_component_obstacle(curr_keepout.area, layer, component_translation, rotation_in_degree, !p_location.is_front, clearance_class, new_component.no, curr_keepout.name, fixed_state);
          }
        }
        else
        {
          ...
          for (int j = 0; j < routing_board.layer_structure.arr.length; ++j)
          {
            if (routing_board.layer_structure.arr[j].is_signal)
            {
              ...
              {
                routing_board.insert_via_obstacle(curr_keepout.area, j, component_translation, rotation_in_degree, !p_location.is_front, clearance_class, new_component.no, curr_keepout.name, fixed_state);
              }
              else
              {
                routing_board.insert_component_obstacle(curr_keepout.area, j, component_translation, rotation_in_degree, !p_location.is_front, clearance_class, new_component.no, curr_keepout.name, fixed_state);
              }
            }
          }
        }
